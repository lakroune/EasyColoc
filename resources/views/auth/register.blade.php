<x-guest-layout>
    <div class="glass-form rounded-3xl p-8 w-full max-w-sm shadow-2xl">
        ...
        <form method="POST" action="{{ route('register') }}" class="space-y-4">
            ...
            <div>
                <input type="text" name="name" value="{{ old('name') }}" required autofocus placeholder="Nom complet *"
                ...
                @error('name') <p class="text-red-500 text-xxs mt-1">{{ $message }}</p> @enderror
            </div>
        ...
        <div class="mt-6 text-center">
            <p class="text-gray-400 text-xxs">
                Déjà inscrit ? 
                <a href="{{ route('login') }}" class="text-teal-600 hover:text-teal-700 font-semibold">Connectez-vous</a>
            </p>
        </div>
    </div>
</x-guest-layout>